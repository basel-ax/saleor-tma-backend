...
describe("fetchRestaurants", () => {
  ...
  it("should fall back to mock restaurants when Saleor is not configured", async () => {
    ...
    expect(result[0]).toEqual({
      ...
      description: TEST_RESTAURANTS.REST_A.description,
      imageUrl: TEST_RESTAURANTS.REST_A.imageUrl,
      tags: TEST_RESTAURANTS.REST_A.tags,
      categories: [],
      deliveryLocations: [],
    });
    expect(result[1]).toEqual({
      ...
      description: TEST_RESTAURANTS.REST_B.description,
      imageUrl: TEST_RESTAURANTS.REST_B.imageUrl,
      tags: TEST_RESTAURANTS.REST_B.tags,
      categories: [],
      deliveryLocations: [],
    });
  });
...
describe("fetchDishes", () => {
  ...
  it("should filter mock dishes by both categoryId and restaurantId when provided", async () => {
    ...
    expect(
      result.every(
        (d) => d.categoryId === "catA" && d.restaurantId === "restA",
      ),
    ).toBe(true);
    expect(result[0].id).toBe(TEST_DISHES.DISH_A1.id);
    expect(result[1].id).toBe(TEST_DISHES.DISH_A2.id);
